...
### Changed
- sanIP 表情包管理体验重构：
  - **两行对称悬浮操作栏**：表情卡片悬浮遮罩操作图标改为整齐的两行网格布局（查看大图、在文件夹中显示、复制图片 / 更换分组、退回普通图片、彻底删除），彻底解决图标拥挤或横向溢出问题。
  - **触发词一键复制**：表情卡片底部快捷词输入框新增悬浮复制按钮，点击即快速将快捷词复制到剪贴板并提示 Toast。
  - **专属拖拽排序手柄**：表情卡片右上角新增拖拽手柄（`GripVertical`），将拖拽触发严格限定在手柄区域，彻底避免点击卡片预览、选中复选框或编辑文本时的误拖拽。
  - **常驻批量操作栏**：表情网格顶部的批量工具栏（全选当前项、已选计数、批量更换分组、批量彻底删除）改为常驻显示，无选中项时批量按钮自动置灰，消除卡片选择过程中的页面高度跳动（Layout Shift）。
