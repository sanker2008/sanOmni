...
use std::path::Path;
use crate::sync::client::{SyncClient, SyncChange, PushRequest};
use uuid::Uuid;
...
pub async fn run_sync(db_path: &str, app: &tauri::AppHandle) -> Result<serde_json::Value, String> {
    ...
    let (server_url, api_key, device_id, last_sync_version, pending_ids, mut changes) = {
        ...
        let server_url: String = conn.query_row("SELECT value FROM sync_config WHERE key = 'server_url'", [], |row| row.get(0))
        ...
        let device_id: String = conn.query_row("SELECT value FROM sync_config WHERE key = 'device_id'", [], |row| row.get(0))
            .unwrap_or_else(|_| {
                let id = Uuid::new_v4().to_string();
                let _ = conn.execute("INSERT INTO sync_config (key, value) VALUES ('device_id', ?)", rusqlite::params![id]);
                id
            });
            
        let last_sync_version: i64 = conn.query_row("SELECT value FROM sync_config WHERE key = 'last_sync_version'", [], |row| {
        ...
        let changes: Vec<SyncChange> = stmt.query_map([], |row| {
        ...
        (server_url, api_key, device_id, last_sync_version, pending_ids, changes)
    ...
    for (i, change) in changes.iter_mut().enumerate() {
        if (change.table == "images" || change.table == "ip_assets" || change.table == "ip_images") && change.operation != "DELETE" {
            if let Some(data_str) = &change.data {
                if let Ok(mut json) = serde_json::from_str::<serde_json::Value>(data_str) {
                    if let Some(abs_path) = json.get("absolute_path").and_then(|v| v.as_str()).map(String::from) {
                        if let Ok(data) = tokio::fs::read(&abs_path).await {
                            ...
                            if let Some(obj) = json.as_object_mut() {
                                obj.insert("file_hash".to_string(), serde_json::Value::String(hash.clone()));
    ...
    if !changes.is_empty() {
        ...
        match client.push(req).await { // <--- 这是一个 .await 点
            Ok(resp) => {
                ...
                for id in pending_ids {
                    let _ = conn.execute("DELETE FROM sync_changelog WHERE id = ?", rusqlite::params![id]);
                }
    ...
    let current_version = {
        let conn = Connection::open(Path::new(db_path)).unwrap();
        conn.query_row("SELECT value FROM sync_config WHERE key = 'last_sync_version'", [], |row| {
            let v: String = row.get(0)?;
            Ok(v.parse::<i64>().unwrap_or(0))
    ...
    let app_root = {
        let conn = Connection::open(Path::new(db_path)).unwrap();
        let unified_root: String = conn.query_row("SELECT value FROM settings WHERE key = 'unifiedRootPath'", [], |r| r.get(0)).unwrap_or_default();
    ...
    match client.pull(current_version).await { // <--- 又一个 .await 点
        Ok(mut resp) => {
                        ...
                        if let Some(data_str) = &change.data {
                            if let Ok(mut json) = serde_json::from_str::<serde_json::Value>(data_str) {
                                if let (Some(hash), Some(rel_path)) = (
                                    json.get("file_hash").and_then(|v| v.as_str()).map(String::from),
                                    ...
                                    if !local_abs_path.exists() {
                                        let _ = client.download_file(&hash, local_abs_path.to_str().unwrap()).await;
                                    }
                                    
                                    if let Some(obj) = json.as_object_mut() {
                                        obj.insert("absolute_path".to_string(), serde_json::Value::String(local_abs_path.to_string_lossy().to_string()));
                                        change.data = Some(serde_json::to_string(&json).unwrap());
                                    }
                                }
                            }
