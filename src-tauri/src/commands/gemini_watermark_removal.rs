...
fn load_alpha_map(size: u32) -> Result<Vec<f32>, String> {
    ...
    let img =
        image::load_from_memory(bytes).map_err(|e| format!("无法解码内置 alpha map: {}", e))?;
...
pub async fn remove_gemini_watermark(
    ...
    for &wm_size in &sizes_to_check {
        ...
        let mut var_a = 0.0;
        for p in &active_pixels {
            var_a += (p.alpha - mean_a) * (p.alpha - mean_a);
        }
    ...
    Ok(GeminiWatermarkRemovalResult {
        ...
        method: format!(
            "gemini_ncc_pixel_reconstruction (size: {}, conf: {:.2})",
            wm_size, best_overall_ncc
        ),
