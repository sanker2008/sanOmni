...
pub async fn create_character(
    ...
    get_character_by_id(app_handle, id)
        .await
        .map(|c| c.character)
}
...
pub async fn update_character(
...
) -> Result<Character, String> {
    {
        ...
        if character_type.is_some() {
            query.push_str(&format!(", character_type = ?{}", param_index));
            param_index += 1;
        }
        ...
        if appearance_info.is_some() {
            query.push_str(&format!(", appearance_info = ?{}", param_index));
            param_index += 1;
        }
        if ip_id.is_some() {
            query.push_str(&format!(", ip_id = ?{}", param_index));
            param_index += 1;
        }
        if ip_relation_note.is_some() {
            query.push_str(&format!(", ip_relation_note = ?{}", param_index));
            param_index += 1;
        }
        ...
        if let Some(ref v) = character_type {
            params_vec.push(v);
        }
        if let Some(ref v) = description {
            params_vec.push(v);
        }
        if let Some(ref v) = appearance_info {
            params_vec.push(v);
        }
        if let Some(ref v) = ip_id {
            params_vec.push(v);
        }
        if let Some(ref v) = ip_relation_note {
            params_vec.push(v);
        }
        params_vec.push(&id);
...
#[tauri::command]
pub async fn delete_character(app_handle: AppHandle, id: String) -> Result<(), String> {
    let conn = get_connection(&app_handle)?;
    let now = Utc::now().to_rfc3339();

    // Get character info before deletion
    let (work_id, image_paths): (String, Option<String>) = conn
        .query_row(
    ...
    if let Some(paths_json) = image_paths {
        if let Ok(paths) = serde_json::from_str::<Vec<String>>(&paths_json) {
            let base_app_data_dir = app_handle
                ...
                .map_err(|e| e.to_string())?;
            let app_data_dir =
                crate::commands::get_works_root_from_handle(&app_handle, &base_app_data_dir);
