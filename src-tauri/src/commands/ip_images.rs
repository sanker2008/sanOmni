...
pub async fn import_ip_image(
    ...
    if let Err(e) = conn.execute(
        ...
        rusqlite::params![
            &ip_image.id,
            &ip_image.filename,
            &ip_image.original_filename,
            &ip_image.ip_id,
            &ip_image.relative_path,
            ...
            &ip_image.file_hash,
            &ip_image.format,
            ip_image.has_watermark as i32,
            &ip_image.watermark_platform,
            ip_image.watermark_detected as i32,
            ip_image.watermark_removed as i32,
            &ip_image.created_at,
            &ip_image.imported_at,
            &ip_image.archived_at,
        ],
    ) {
...
pub async fn update_ip_image(
    ...
    let library_path = if current_image
        .absolute_path
        .ends_with(&current_image.relative_path)
    {
        let len = current_image.absolute_path.len() - current_image.relative_path.len();
        current_image.absolute_path[..len]
            .trim_end_matches('/')
            .trim_end_matches('\\')
            .to_string()
    } else {
    ...
    let new_ip_path =
        fetch_ip_path(&conn, &request.primary_ip_id).unwrap_or_else(|| "unknown".to_string());
    ...
    let need_rename = current_image.status == "archived"
        && (is_ip_changed || is_filename_invalid || is_path_mismatched);

    if need_rename && !library_path.is_empty() {
        let target_dir = std::path::PathBuf::from(&library_path)
            .join("ip_archived")
            .join(&new_ip_path);
        ...
        if source.exists() {
            if let Ok(_) = std::fs::create_dir_all(&target_dir) {
                if std::fs::rename(source, &target_path)
                    .or_else(|_| {
    ...
    if current_image.status == "archived" {
        if let Some(pack_id) = &request.associate_sticker_pack_id {
            if !pack_id.trim().is_empty() {
                ...
                let (mut current_absolute_path, _, mut current_filename) = conn
                    .query_row(
                    ...
                    .unwrap_or_else(|_| {
                        (
                            current_image.absolute_path.clone(),
                            current_image.relative_path.clone(),
                            current_image.filename.clone(),
                        )
                    });

                // Fetch pack path and IP path
                let ip_path = fetch_ip_path(&conn, &request.primary_ip_id)
                ...
                if let Some(data_dir) = db_path_obj.parent() {
                    if let Some(base_app_data_dir) = data_dir.parent() {
                        ...
                        let stem = path_candidate
                            .file_stem()
                            .and_then(|s| s.to_str())
                            .unwrap_or(&filename_candidate)
                            .to_string();
                        let current_ext = path_candidate
                            .extension()
                            .and_then(|e| e.to_str())
                            .unwrap_or(ext)
                            .to_string();
                        let mut unique_filename = path_candidate
                            .file_name()
                            .and_then(|f| f.to_str())
                            .unwrap_or(&filename_candidate)
                            .to_string();
                        ...
                        if source.exists() && source != target_path {
                                ...
                                .is_ok()
                            {
                                ...
                                let existing_emoji_id: Option<String> = conn
                                    .query_row(
    ...
    for ip_id in &request.ip_ids {
        let is_primary = if ip_id == &request.primary_ip_id {
            1
        } else {
            0
        };
    ...
    let ip_ids = if ip_ids.is_empty() {
        vec![ip_image.ip_id.clone()]
    } else {
        ip_ids
    };
...
pub async fn archive_ip_images(
    ...
    for ip_image_id in &request.ip_image_ids {
        ...
        // Get IP path
        let ip_path =
            fetch_ip_path(&conn, &ip_image.ip_id).unwrap_or_else(|| "unknown".to_string());
        ...
        let (target_path, new_filename) = loop {
            ...
            let stem = path_file
                ...
                .unwrap_or(&filename_candidate)
                .to_string();
            let current_ext = path_file
                .extension()
                .and_then(|e| e.to_str())
                .unwrap_or(ext)
                .to_string();

            let final_candidate = if !template.contains("{time}")
            ...
            let check_stem = if !template.contains("{time}")
                && !template.contains("{index}")
                && suffix_counter > 1
            {
        ...
        if source.exists() {
            ...
            if let Err(e) = std::fs::rename(source, &target_path).or_else(|_| {
                ...
                result
                    .errors
                    .push(format!("{}: Failed to move file: {}", ip_image_id, e));
                continue;
            }
        }
        ...
        if let Err(e) = conn.execute(
            ...
            result
                .errors
                .push(format!("{}: Failed to update database: {}", ip_image_id, e));
            continue;
        }
...
fn fetch_ip_image_by_id(conn: &Connection, ip_image_id: &str) -> Option<IpImage> {
    ...
    stmt.query_row([ip_image_id], |row| {
        Ok(IpImage {
            ...
            format: row.get(11)?,
            has_watermark: row.get::<_, Option<i32>>(12)?.unwrap_or(0) != 0,
            watermark_platform: row.get(13)?,
            watermark_detected: row.get::<_, Option<i32>>(14)?.unwrap_or(0) != 0,
            watermark_removed: row.get::<_, Option<i32>>(15)?.unwrap_or(0) != 0,
    ...
    match rows {
        Ok(mapped) => mapped
            ...
            .map(|ip_image| {
                ...
                let ip_name =
                    fetch_ip_name(conn, &ip_image.ip_id).unwrap_or_else(|| "Unknown".to_string());
...
fn fetch_ip_name(conn: &Connection, ip_id: &str) -> Option<String> {
    conn.query_row("SELECT name FROM ip_assets WHERE id = ?", [ip_id], |row| {
        row.get(0)
    })
    .ok()
}
...
fn fetch_ip_path(conn: &Connection, ip_id: &str) -> Option<String> {
    conn.query_row("SELECT path FROM ip_assets WHERE id = ?", [ip_id], |row| {
        row.get(0)
    })
    .ok()
}
...
fn fetch_associated_ips(conn: &Connection, ip_image_id: &str) -> Result<Vec<String>> {
    let mut stmt = conn.prepare("SELECT ip_id FROM ip_image_relations WHERE ip_image_id = ?")?;
...
pub async fn update_ip_image_file(
    ...
    let old_absolute_path: Option<String> = conn
        .query_row(
    ...
    if let Some(old_path) = old_absolute_path {
        ...
        let _ = conn.execute(
            "UPDATE ip_assets SET avatar_path = ? WHERE avatar_path = ?",
            [new_path, &old_path],
        );
        let _ = conn.execute(
            "UPDATE ip_creations SET image_path = ? WHERE image_path = ?",
            [new_path, &old_path],
        );
        let _ = conn.execute(
            "UPDATE ip_emojis SET image_path = ? WHERE image_path = ?",
            [new_path, &old_path],
        );
        let _ = conn.execute(
            "UPDATE ip_character_sheets SET image_path = ? WHERE image_path = ?",
            [new_path, &old_path],
        );
        let _ = conn.execute(
