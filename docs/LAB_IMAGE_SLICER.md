# sanLabs — 图片切割工具 (Image Slicer) Specs
...
sanLabs 是 sanOmni 的第三个顶级功能域，是一个独立的实验性工具集合区域。**图片切割 (Image Slicer)** 是 sanLabs 中的第二个工具，能够帮助用户以极其灵活、可视化的方式对图片进行均等分或自定义切分，支持双参考线间隔、鼠标拖拽调整、保留/删除切片选择、按比例缩放适配、自定义命名及格式，并能一键批量导出到指定系统目录。
