...
#### 自定义作品集根目录
- **作用：** 作品集及其角色图库的独立存储位置
- **默认：** `C:\Users\<用户名>\AppData\Roaming\com.sanomni.app\works`
- **设置方法：**
  1. 进入"IP 设置"标签页
  2. 找到"自定义作品集根目录"配置项
  3. 输入路径或浏览选择，留空则使用统一根目录或默认位置
...
在引入“统一根目录”概念后，系统的存储路径优先级如下：

```
特定子目录自定义路径 (如 customInboxPath) > 统一根目录 (Unified Root Path) > 系统默认路径 (AppData)
```

1. **特定子目录**：如果您在具体的功能（如 IP 设定、归档）中指定了专属的路径，系统会最高优先级使用它。
2. **统一根目录**：如果您未设置具体的专属路径，但配置了“统一根目录”，所有占用大量空间的多媒体数据都会存放到 `{统一根目录}/对应子模块` 文件夹中。
3. **系统默认**：如果均未设置，系统会使用操作系统的 `AppData` 目录。

> **注意**：数据库文件 (`database.sqlite`) 为了保证应用启动的可靠性，无论如何配置，都会**固定存储在系统默认的 AppData 目录**下。只有大体积文件才会跟随上述优先级发生转移。
