...
- 极端宽高比盲区（2026-08-13 修复）：对于长竖图（如 `704x5856`）或横 Banner（如 `2560x800`），虽然单边像素极大，但由于其中一边小于等于 1024，原判定条件 `width > 1024 && height > 1024` 会误判其为“小图”，跳过 `96px / margin 192` 匹配。
...
4. **已知 Profile 判定限制为双边 > 1024**：长竖图/横 Banner 因单边 <= 1024 无法命中大图 96px Profile；同时旧 `search_margin` 搜索范围（最小 128px）无法覆盖 96px + margin 192 所需的 288px 偏移。
...
- `704x5856` 极长图与 `2560x800` 极宽图：误匹配到右下角电路板纹理的 `48px / margin 32` 假峰值，真实的 `96px` 水印被完好保留。
...
- 内置 alpha 模板：
  ...
  - `bg_96_20260520.png`
- 检查尺寸：`96 / 72 / 48 / 36`。
- 新 profile 优先于自由搜索（基于 `width.max(height) > 1024` 判断大图）：
  - 大图/长图/宽图：优先尝试 `96px / margin 192 / alpha 20260520`，随后尝试 `48px / margin 96 / legacy_scale_0.60` 及 legacy 组合。
  - 小图：优先尝试 `48px / margin 96 / legacy_scale_0.60`。
...
- 自由搜索 `search_margin` 安全边距扩充至 `clamp(288, 512)`，确保能够完全覆盖 `96px` 水印在窄图中的内缩范围。
- 旧 profile 和自由搜索作为 fallback：
  - 大图 legacy：`96px`，右/下 `192px`（alpha 20260520）。
  - 非大图 legacy：`48px`，右/下 `32px`（alpha Legacy）。
...
| 大图/长图/宽图新版，黑底残留、变深、亮边 | `20260520` | `96px / margin 192` |
...
- **`704x5856` 竖向极长图已知 `96px / margin 192` Profile 优先命中**。
- **`2560x800` 横向极宽图已知 `96px / margin 192` Profile 优先命中**。
