# 文档索引

> sanOmni 包含 **Prompt 模板管理器** 和 **IP 形象管理器** 两大功能域，文档按域分类组织。

## 🏗️ 架构设计

- [架构文档](./ARCHITECTURE.md) - 说明 sanPrompt, sanIP 与 sanLabs 的模块化设计、模块划分与未来拆分规划

## 📖 通用文档

### 使用指南
- [使用指南](./USAGE.md) - 应用功能使用说明
- [故障排除](./TROUBLESHOOTING.md) - 常见问题解决
- [开发进度](./PROGRESS.md) - 项目进度

### 存储相关
- [存储结构](./STORAGE_STRUCTURE.md) - 文件存储位置
- [数据持久化](./DATA_PERSISTENCE.md) - 数据备份迁移
- [自定义存储路径](./CUSTOM_STORAGE_PATH.md) - 自定义路径

### 通用功能
- [删除功能](./DELETE_FEATURE.md) - 图片删除
- [图片查看器](./IMAGE_VIEWER.md) - 全屏查看
- [文件夹监控](./FOLDER_MONITORING.md) - 自动导入
- [打开文件夹和归档](./OPEN_FOLDER_AND_ARCHIVE.md) - 右键功能
- [水印检测算法](./WATERMARK_DETECTION_ALGORITHM.md) - 水印检测
- [水印检测测试](./WATERMARK_DETECTION_TESTING.md) - 测试指南

## 📝 sanPrompt

- [Prompt 对比](./PROMPT_COMPARISON.md) - Prompt 对比功能
- [Prompt 对比快速开始](./PROMPT_COMPARISON_QUICKSTART.md) - 5 分钟上手
- [Prompt 筛选](./PROMPT_FILTER_FEATURE.md) - 筛选功能
- [厂商管理](./VENDOR_MANAGEMENT.md) - 厂商和模型管理

## 🎭 sanIP

> IP 形象管理相关文档正在建设中，功能已上线，详细文档后续补充。

## 🧪 sanLabs 实验室

- [产品图制作](./LAB_PRODUCT_IMAGE_MAKER.md) - 产品图推广制作工具
- [图片切割](./LAB_IMAGE_SLICER.md) - 多功能可视化图片切割工具

## 🔧 开发文档

- [按需打包配置](./ON_DEMAND_PACKAGING.md) - 配置环境变量进行功能裁剪打包 🆕
- [自动更新发布](./AUTO_UPDATE.md) - GitHub Actions 全自动发版指南 🆕
- [跨平台支持](./CROSS_PLATFORM.md) - 平台支持说明
- [平台修复](./PLATFORM_FIXES.md) - 平台问题修复

---

**最后更新**: 2026-06-01
