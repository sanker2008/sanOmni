...
- [技术栈全景](#技术栈全景)
- [双域架构设计](#双域架构设计)
- [App A: Prompt 模板管理器 (sanPromptBox)](#app-a-prompt-模板管理器-sanpromptbox)
- [App B: IP 形象管理器 (sanIPBox)](#app-b-ip-形象管理器-sanipbox)
...
sanMediaBox 是一个基于 Tauri 2.0 的跨平台桌面应用程序，当前**包含两个独立的功能领域**，以一个统一的应用形式打包发布，但在架构设计上**保持领域分离**，以便未来可以轻松拆分为两个独立应用。
...
3. **资源共享**: 共享 Tauri 运行时、SQLite 实例、文件系统监控等

### 为什么要保持可拆分？

1. **用户群分化**: Prompt 管理和 IP 管理可能面向不同用户群体
...
## App A: Prompt 模板管理器 (sanPromptBox)

### 核心定位

**Prompt 模板管理**是 App A 的核心目的。图片（inbox/archived）的存在是为了**服务于 Prompt 模板**——记录同一 Prompt 在不同 AI 模型下的生成效果，进行跨模型比较。
...
| **Prompt 模板** | 创建 Prompt 模板，支持模板变量（template_schema）和插值渲染 |
| **跨模型对比** | 将同一 Prompt 的不同模型生成图绑定到一个 PromptGroup，对比效果 |
...
└────────┬─────────┘
         ▼
┌──────────────────┐
│  关联 PromptGroup │ ← 可选：将图片绑定到 Prompt 模板
│  跨模型效果对比    │
└──────────────────┘
```
...
| 文件 | 命令数 | 职责 |
|------|--------|------|
| `ip_assets.rs` | **21** | IP 资产全生命周期管理（CRUD、设定图、创作、贴纸包、表情、平台、关系） |
...
| `get_ip_asset_detail` | 获取单个 IP 角色的完整详情（含关联数据） |
| `create_ip_asset` | 创建新 IP 角色 |
| `update_ip_asset` | 更新 IP 角色信息 |
...
IP 模块当前没有独立 Store，仅在 `useUIStore` 中维护 `selectedIpId` 状态，IP 数据通过组件内 `useState` 直接管理。
...
| **文件监控** | useFolderWatcher.ts | watcher.rs | 文件夹变化监听（基于 notify crate） |
...
│   ├── useFolderWatcher.ts        [5KB] # 共享: 文件夹监控
...
| image_prompt_group_relations | (image_id, prompt_group_id) | 图片与 Prompt 组的多对多关系 |
...
| idx_itr_tag | image_tag_relations | tag_id | 标签关联查询 |
...
tauri::Builder::default()
    ...
    .manage(commands::watcher::WatcherState::new())  // 文件监控状态
    .setup(|app| {
        // 1. 获取应用数据目录
        // 2. 创建 data/ 子目录
...
| Prompt ↔ Image | 外键关联 | 🔴 **紧耦合** | `image_prompt_group_relations` 有外键，这是**有意设计** |
...
2. **Prompt 组 与 Image 的紧耦合**

   ```sql
   -- Prompt 组关联：通过外键强关联到 images 表
   ...
   这也是有意设计——图片存在的意义就是服务于 Prompt 模板，两者天然是紧耦合的。
...
当需要将 sanMediaBox 拆分为两个独立应用时，遵循以下策略：
