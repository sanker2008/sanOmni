...
系统当前**包含四个独立的功能领域**，以一个统一的应用形式打包发布：

1. **sanPrompt (Prompt 模板商品管理)**：专注于结构化管理、跨模型验证和商业化发布 AI 绘画 Prompt 模板，管理生成的灵感图库与商品素材。
2. **sanIP (IP 资产管理)**：专注于角色 IP 设定、三视图、衍生作品、表情包分发和状态跟踪。
3. **sanLabs 工具箱 (Product Image Maker 等)**：提供独立、零后端依赖的客户端本地图片处理工具箱，目前内置强大的“产品图制作”画板功能，支持图层管理、模板保存与图片合成。
4. **sanKnow 开发知识库**：在本地索引项目代码、文档和配置，支持沉淀手动结论与收录公开网页文档，帮助后续开发快速回到可追溯的原始来源。

这四大功能域在架构设计上**保持领域分离**，确保代码高内聚、低耦合。
...
### 📚 sanKnow 开发知识库
- 🗂️ **项目索引** - 本地收录项目中的文档、代码和配置，并保留来源路径
...
- ☁️ **高级云端同步** - 内置极速增量同步引擎，配套专属私有化服务端 `sanomni-sync-server`。支持指定 IP 粒度推送、精细化的单向推送/拉取控制、全聚合的“人类可读”同步时间轴、一键强制全量覆盖、以及智能的文件垃圾回收保护 🆕
...
- ✅ 模块化的设置页面
- ✅ 路径迁移工具与数据自修复
- ✅ **全量与按 IP 精细化云端同步** - 支持按指定 IP 筛选推送、单向推送/拉取控制，提供聚合时间轴的同步历史面板 (需搭配私有项目 `sanomni-sync-server` 使用) 🆕
