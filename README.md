...
> 前称为 sanMediaBox，现已升级并更名为 **sanOmni**，意为全方位的 AI 创作辅助中心。
...
1. **sanPromptBox (Prompt 模板管理器)**：专注于结构化管理和对比 AI 绘画的 Prompt 模板，管理生成的灵感图库。
2. **sanIPBox (IP 形象资产管理器)**：专注于角色 IP 设定、三视图、衍生作品、表情包分发和状态跟踪。
3. **Labs 工具箱 (Product Image Maker 等)**：提供独立、零后端依赖的客户端本地图片处理工具箱，目前内置强大的“产品图制作”画板功能，支持图层管理、模板保存与图片合成。
...
- ⚙️ **模块化设置** - 重构的现代化设置面板，按域划分（通用、Prompt、IP 资产、Labs 等）
- ⌨️ **快捷键** - 丰富的键盘快捷键支持
- 💾 **自定义路径** - 灵活自定义存储位置与 Labs 工作目录
- 📦 **按需打包** - 通过环境变量支持隔离打包出“纯 Prompt 模板管理器”专属版本，自动精简界面和体积
...
> 📐 **架构说明：** 项目采用三域架构，Prompt 模板管理、IP 形象管理与 Labs 工具箱三个功能域在数据库、命令和前端组件层面相互独立，便于未来拆分为独立应用。详细见 [架构文档](./docs/ARCHITECTURE.md)。
...
**最后更新：** 2026-05-29  
**版本：** 2.1.0  
**最新功能：** 全新更名为 sanOmni、新增 Labs 工具箱（内置产品图海报制作工具）、模块化重构设置面板 🆕
