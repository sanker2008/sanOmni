...
import { Check, X, CheckSquare, Square, Pencil, Eraser } from 'lucide-react';
...
interface SliceGridPreviewProps {
  ...
  onEditSlice: (slice: SliceItem, index: number) => void;
}
    ...
    <div className="flex-1 h-full flex flex-col bg-slate-900/10 dark:bg-black/10 overflow-hidden">
      ...
      <div className="flex items-center justify-between px-6 py-3 border-b border-border bg-card/60 shrink-0">
        <div className="flex items-center gap-2.5">
          <h3 className="text-sm font-semibold">切片选择与微调</h3>
            ...
            {slices.map((slice, index) => {
              ...
              return (
                ...
                >
                  {/* Thumbnail Area */}
                  <div
                    className="w-full h-[130px] flex items-center justify-center bg-slate-950/20 dark:bg-black/30 relative border-b border-border/40 select-none cursor-pointer"
                  ...
                  >
                    ...
                    <div
                      className={`relative overflow-hidden transition-transform duration-200 group-hover:scale-[1.03] ${
                        outputBg === 'transparent'
                          ? 'bg-[linear-gradient(45deg,#e5e7eb_25%,transparent_25%),linear-gradient(-45deg,#e5e7eb_25%,transparent_25%),linear-gradient(45deg,transparent_75%,#e5e7eb_75%),linear-gradient(-45deg,transparent_75%,#e5e7eb_75%)] bg-[length:12px_12px] bg-[position:0_0,0_6px,6px_-6px,-6px_0px] dark:bg-[linear-gradient(45deg,#374151_25%,transparent_25%),linear-gradient(-45deg,#374151_25%,transparent_25%),linear-gradient(45deg,transparent_75%,#374151_75%),linear-gradient(-45deg,transparent_75%,#374151_75%)]'
                          : ''
                      }`}
                    ...
                    >
                      {slice.customDataUrl ? (
                        /* Custom edited canvas image */
                        <img
                          src={slice.customDataUrl}
                          alt={`Slice #${index + 1}`}
                    ...
                    {slice.selected && (
                      <div
                        className="absolute top-2 right-2 bg-emerald-500 text-white rounded-full p-0.5 shadow-sm hover:scale-110 transition-transform"
                        onClick={(e) => {
                          e.stopPropagation();
                          onToggleSelect(slice.id);
                        }}
                      >
                    ...
                    {/* Edited Badge */}
                    {slice.isEdited && (
                      <div className="absolute top-2 left-2 bg-blue-600 text-white font-medium text-[8px] px-1 py-0.2 rounded shadow-sm flex items-center gap-0.5">
                    ...
                    <button
                      ...
                      className="absolute bottom-1.5 right-2 opacity-0 group-hover:opacity-100 transition-opacity bg-primary text-primary-foreground text-[10px] px-2 py-0.5 rounded shadow flex items-center gap-1 font-medium hover:bg-primary/90"
                      title="点击进入单独编辑 (位置微调/橡皮擦)"
                    >
                  ...
                  {/* Meta Label Info */}
                  <div className="w-full p-2.5 flex flex-col gap-1 bg-card/40">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <button
                        type="button"
                        onClick={() => onEditSlice(slice, index)}
